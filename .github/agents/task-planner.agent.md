...
- Prefer vertical slices: implement one user-visible behaviour end-to-end rather than separate backend/frontend tasks unless necessary.
 ...
 - Docs-first enforcement: Do NOT emit implementation tasks unless both `stories.md` and `requirements.md` exist for the feature and contain acceptance criteria (and tests when required by the project rules). If either file is missing or incomplete, the agent MUST produce exactly one docs task (Task 01 — Spec: Stories & Requirements) and stop; coding tasks are only generated after Task 01 is completed.
...
- Suggest testing strategy and fixtures for integration tasks.
 - Validate prerequisites: before emitting a `tasks.md`, verify that `specs/[NN-...]/stories.md` and `specs/[NN-...]/requirements.md` exist and include acceptance criteria. If validation fails, create Task 01 (docs) and treat it as a blocking prerequisite.
