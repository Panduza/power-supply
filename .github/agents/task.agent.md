...
- Map tasks back to the originating `specs/` and `1-requirements.md` entry (include a `specs/` path reference). Use `0-stories.md` for stories and `1-requirements.md` for requirements.
- Prefer vertical slices: implement one user-visible behaviour end-to-end rather than separate backend/frontend tasks unless necessary.
 ...
 - Docs-first enforcement: Do NOT emit implementation tasks unless both `0-stories.md` and `1-requirements.md` exist for the feature and contain acceptance criteria (and tests when required by the project rules). If either file is missing or incomplete, the agent MUST produce exactly one docs task (Task 01 — Spec: Stories & Requirements) and stop; coding tasks are only generated after Task 01 is completed.

# Responsibilities

- Read the `1-requirements.md` and create a `2-tasks.md` that decomposes every functional requirement into concrete tasks.
...
- Suggest testing strategy and fixtures for integration tasks.
 - Validate prerequisites: before emitting a `2-tasks.md`, verify that `specs/[NN-...]/0-stories.md` and `specs/[NN-...]/1-requirements.md` exist and include acceptance criteria. If validation fails, create Task 01 (docs) and treat it as a blocking prerequisite.
