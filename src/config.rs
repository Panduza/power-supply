...
impl GlobalConfig {
    ...
    pub fn from_user_file() -> Self {
        ...
        match std::fs::read_to_string(&config_path) {
            Ok(content) => {
                match serde_json5::from_str::<GlobalConfig>(&content) {
                    Ok(config) => config,
                    Err(err) => {
                        error!("Failed to parse config file: {}. Generating default configuration.", err);
    ...
    fn generate_default_config(config_path: &Path) -> Self {
        ...
        if let Err(err) = std::fs::write(config_path, config_content) {
            ...
            info!("Generated default configuration file at: {}", config_path.display());
        }
